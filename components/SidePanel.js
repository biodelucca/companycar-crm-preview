...
import { formatarDataHoraVenda } from "../utils/vendaRealizada.js";
...
export function SidePanel({ oportunidade, cliente, responsavel, usuarios = [], etapas, etapaAtual, motivosPerda = [], origens = [], timelineEventos, onFechar, onMoverEtapa, onReabrir, onReagendarVisita, onTransferir, onChecklistMarcado, onAssociarVeiculoEstoque, onEditarDados, onExcluir, onSalvarProximaAcao, onConcluirProximaAcao, }) {
    ...
    // deliberada e separada da movimentação normal acima: só aparece quando
    // etapaAtual.tipo === "perdido" (ver JSX da seção "Mover para outra
    // etapa" mais abaixo). etapaEhFinal continua controlando o caso "ganho"
    // (texto estático "não pode ser alterada", sem nenhuma mudança).
    const etapaEhPerdido = etapaAtual?.tipo === "perdido";
    const [etapaReaberturaAlvo, setEtapaReaberturaAlvo] = useState("");
    const [reabrindo, setReabrindo] = useState(false);
    const [erroReabertura, setErroReabertura] = useState(null);
    async function confirmarReabertura() {
        if (!etapaReaberturaAlvo)
            return;
        setReabrindo(true);
        setErroReabertura(null);
        const resultado = await onReabrir(etapaReaberturaAlvo);
        setReabrindo(false);
        if (resultado && resultado.ok === false) {
            setErroReabertura(resultado.erro ?? "Não foi possível reabrir a oportunidade agora.");
            return;
        }
        setEtapaReaberturaAlvo("");
    }
    // Melhoria isolada "Visita Agendada com data e hora" (2026-08-24) —
    // reagendamento da visita já marcada. Mesmo padrão de
    // etapaReaberturaAlvo/confirmarReabertura acima (ação dedicada, separada
    // da movimentação normal), só aparece quando a oportunidade já está em
    // "Visita Agendada" (ver JSX da nova seção "Visita agendada" abaixo).
    const [editandoVisita, setEditandoVisita] = useState(false);
    const [reagendarData, setReagendarData] = useState("");
    const [reagendando, setReagendando] = useState(false);
    const [erroReagendar, setErroReagendar] = useState(null);
    function iniciarEdicaoVisita() {
        setReagendarData(oportunidade.visitaAgendadaEm ?? "");
        setErroReagendar(null);
        setEditandoVisita(true);
    }
    async function confirmarReagendarVisita() {
        if (!reagendarData) {
            setErroReagendar("Selecione a data e o horário da visita.");
            return;
        }
        setReagendando(true);
        setErroReagendar(null);
        const resultado = await onReagendarVisita(reagendarData);
        setReagendando(false);
        if (resultado && resultado.ok === false) {
            setErroReagendar(resultado.erro ?? "Não foi possível reagendar a visita agora.");
            return;
        }
    ...
    // padrão de precisaMotivo/motivoAlvo acima, para o caso de mover PARA
    // "Visita Agendada" pelo seletor por botão (mesmo caminho usado pelo
    // drag-and-drop em Pipeline.tsx, ver moverEtapa lá — validação real é
    // sempre do backend).
    const precisaVisita = etapaAlvoObj?.nome === "Visita Agendada";
    const [visitaAlvoData, setVisitaAlvoData] = useState("");
    async function confirmarMovimento() {
        ...
        if (precisaOutroTexto && !motivoOutroAlvo.trim())
            return;
        if (precisaVisita && !visitaAlvoData)
            return;
        setMovendoEtapa(true);
        setErroMovimento(null);
        const resultado = await onMoverEtapa(etapaAlvo, precisaMotivo ? motivoAlvo : undefined, precisaOutroTexto ? motivoOutroAlvo.trim() : undefined, precisaVisita ? visitaAlvoData : undefined);
    ...
    return (_jsxs("div", { className: "side-panel__overlay", onClick: onFechar, children: [_jsxs("aside", { className: "side-panel", onClick: (e) => e.stopPropagation(), children: [_jsxs("div", { className: "side-panel__header", children: [_jsxs("div", { children: [_jsx("h2", { children: oportunidade.veiculoInteresse }), _jsx("p", { className: "side-panel__cliente", children: cliente?.nome ?? "Cliente não identificado" })] }), _jsx("button", { className: "side-panel__fechar", onClick: onFechar, "aria-label": "Fechar", children: "\u2715" })] }), _jsxs("div", { className: "side-panel__tabs", children: [_jsx("button", { className: aba === "detalhes" ? "ativo" : "", onClick: () => setAba("detalhes"), children: "Detalhes" }), _jsx("button", { className: aba === "timeline" ? "ativo" : "", onClick: () => setAba("timeline"), children: "Timeline" }), _jsx("button", { className: aba === "checklist" ? "ativo" : "", onClick: () => setAba("checklist"), children: "Checklist" })] }), _jsxs("div", { className: "side-panel__body", children: [aba === "detalhes" && (_jsxs(_Fragment, { children: [_jsxs("dl", { className: "side-panel__lista", children: [_jsx("dt", { children: "Etapa" }), _jsx("dd", { children: etapaAtual?.nome ?? "—" }), _jsx("dt", { children: "Respons\u00E1vel" }), _jsx("dd", { children: responsavel?.nome ?? "—" }), _jsx("dt", { children: "Origem" }), _jsx("dd", { children: origemObj?.nome ?? "—" }), _jsx("dt", { children: "Telefone" }), _jsx("dd", { children: cliente?.telefone ?? "—" }), _jsx("dt", { children: "Cidade" }), _jsx("dd", { children: cliente?.cidade ?? "—" }), _jsx("dt", { children: "Data de in\u00EDcio" }), _jsx("dd", { children: formatarDataSimples(oportunidade.dataInicioNegociacao || oportunidade.criadoEm) }), (etapaAtual?.nome === "Visita Agendada" || oportunidade.visitaAgendadaEm) && (_jsxs(_Fragment, { children: [_jsx("dt", { children: "Visita agendada" }), _jsx("dd", { children: oportunidade.visitaAgendadaEm
                                                        ...
                                                        return (_jsxs(_Fragment, { children: [imagem && (_jsx("img", { className: "side-panel__veiculo-estoque-imagem", src: imagem, alt: modeloVersao ?? "Veículo" })), _jsxs("dl", { className: "side-panel__lista", children: [_jsx("dt", { children: "Ve\u00EDculo" }), _jsx("dd", { children: [marca, modeloVersao, ano].filter(Boolean).join(" ") || "—" }), _jsx("dt", { children: "Km" }), _jsx("dd", { children: km !== null && km !== undefined ? `${km.toLocaleString("pt-BR")} km` : "—" }), _jsx("dt", { children: "Pre\u00E7o" }), _jsx("dd", { children: preco ? formatoMoeda.format(preco) : "Sem preço informado" })] })] }));
                                                            ...
                                                            setAnotacoesSalvoAgora(false);
                                                        }, placeholder: "Observa\u00E7\u00F5es internas sobre esta oportunidade\u2026", rows: 4 }), anotacoesErro && _jsx("p", { className: "side-panel__aviso", children: anotacoesErro }), _jsxs("div", { className: "side-panel__form-acoes", children: [_jsx("button", { className: "side-panel__botao-primario", onClick: handleSalvarAnotacoes, disabled: anotacoesSalvando || anotacoesTexto === anotacoesSalvo, children: anotacoesSalvando ? "Salvando…" : "Salvar" }), anotacoesSalvoAgora && !anotacoesSalvando && (_jsx("span", { className: "side-panel__anotacoes-status", children: "Salvo" }))] })] }))] }), _jsxs("div", { className: "side-panel__secao", children: [_jsx("h3", { className: "side-panel__secao-titulo", children: "Pr\u00F3xima a\u00E7\u00E3o" }), !editandoProximaAcao ? (_jsxs(_Fragment, { children: [_jsx("div", { className: "side-panel__proxima-acao-atual", children: temAcaoAtiva ? (_jsxs(_Fragment, { children: [_jsx("p", { className: "side-panel__proxima-acao-descricao", children: descricaoAcaoAtual }), _jsxs("p", { className: "side-panel__proxima-acao-meta", children: [oportunidade.proximaAcaoData ? formatarDataEvento(oportunidade.proximaAcaoData) : null, " — Responsável: ", nomeResponsavelAcao] })] })) : (_jsx("p", { className: "side-panel__vazio-aba", children: "Nenhuma próxima ação definida" })) }), _jsxs("div", { className: "side-panel__form-acoes", children: [_jsx("button", { className: "side-panel__botao-secundario", onClick: iniciarEdicaoProximaAcao, children: temAcaoAtiva ? "Editar próxima ação" : "Definir próxima ação" }), temAcaoAtiva && (_jsx("button", { className: "side-panel__botao-primario", onClick: handleConcluirProximaAcao, disabled: concluindoAcao, children: concluindoAcao ? "Concluindo…" : "✔ Concluir" }))] })] })) : (_jsxs("div", { className: "side-panel__form", children: [_jsxs("label", { className: "side-panel__campo", children: [_jsx("span", { children: "Tipo da a\u00E7\u00E3o" }), _jsxs("select", { value: paTipo, onChange: (e) => setPaTipo(e.target.value), children: [_jsx("option", { value: "", children: "Selecione\u2026" }), TIPOS_PROXIMA_ACAO.map((t) => (_jsx("option", { value: t, children: t }, t)))] })] }), paTipo === "Outro" && (_jsxs("label", { className: "side-panel__campo", children: [_jsx("span", { children: "Descreva a a\u00E7\u00E3o" }), _jsx("input", { type: "text", value: paOutroTexto, onChange: (e) => setPaOutroTexto(e.target.value) })] })), _jsxs("label", { className: "side-panel__campo", children: [_jsx("span", { children: "Data e hora" }), _jsx("input", { type: "datetime-local", value: paData, onChange: (e) => setPaData(e.target.value) })] }), _jsxs("label", { className: "side-panel__campo", children: [_jsx("span", { children: "Respons\u00E1vel pela a\u00E7\u00E3o" }), _jsx("select", { value: paResponsavelId, onChange: (e) => setPaResponsavelId(e.target.value), children: usuarios.map((u) => (_jsx("option", { value: u.id, children: u.nome }, u.id))) })] }), erroProximaAcao && _jsx("p", { className: "side-panel__aviso", children: erroProximaAcao }), _jsxs("div", { className: "side-panel__form-acoes", children: [_jsx("button", { className: "side-panel__botao-primario", onClick: salvarProximaAcaoForm, disabled: salvandoProximaAcao, children: salvandoProximaAcao ? "Salvando…" : "Salvar" }), _jsx("button", { className: "side-panel__botao-secundario", onClick: () => setEditandoProximaAcao(false), disabled: salvandoProximaAcao, children: "Cancelar" })] })] })), !editandoProximaAcao && erroProximaAcao && _jsx("p", { className: "side-panel__aviso", children: erroProximaAcao })] }), etapaAtual?.nome === "Visita Agendada" && (_jsxs("div", { className: "side-panel__secao", children: [_jsx("h3", { className: "side-panel__secao-titulo", children: "Visita agendada" }), !editandoVisita ? (_jsxs(_Fragment, { children: [_jsx("p", { className: "side-panel__proxima-acao-descricao", children: oportunidade.visitaAgendadaEm
                                                    ? `Visita agendada: ${formatarDataHoraVisita(oportunidade.visitaAgendadaEm)}`
                                                    : "Visita agendada — data/hora não informada" }), _jsx("div", { className: "side-panel__form-acoes", children: _jsx("button", { className: "side-panel__botao-secundario", onClick: iniciarEdicaoVisita, children: oportunidade.visitaAgendadaEm ? "Reagendar visita" : "Informar data/hora" }) })] })) : (_jsxs("div", { className: "side-panel__form", children: [_jsxs("label", { className: "side-panel__campo", children: [_jsx("span", { children: "Data e horário da visita *" }), _jsx("input", { type: "datetime-local", value: reagendarData, onChange: (e) => setReagendarData(e.target.value) })] }), erroReagendar && _jsx("p", { className: "side-panel__aviso", children: erroReagendar }), _jsxs("div", { className: "side-panel__form-acoes", children: [_jsx("button", { className: "side-panel__botao-primario", onClick: confirmarReagendarVisita, disabled: reagendando, children: reagendando ? "Salvando…" : "Salvar" }), _jsx("button", { className: "side-panel__botao-secundario", onClick: () => setEditandoVisita(false), disabled: reagendando, children: "Cancelar" })] })] }))] })), _jsxs("div", { className: "side-panel__secao", children: [_jsx("h3", { className: "side-panel__secao-titulo", children: "Mover para outra etapa" }), etapaEhPerdido ? (_jsxs("div", { className: "side-panel__form", children: [_jsx("p", { className: "side-panel__aviso", children: "Esta negocia\u00E7\u00E3o est\u00E1 em Perdido. Use \"Reabrir oportunidade\" para voltar ao Pipeline ativo \u2014 o hist\u00F3rico da perda \u00E9 preservado." }), _jsxs("select", { value: etapaReaberturaAlvo, onChange: (e) => {
                                                            setEtapaReaberturaAlvo(e.target.value);
                                                            setErroReabertura(null);
                                                        }, children: [_jsx("option", { value: "", children: "Selecione a etapa de retorno\u2026" }), etapas
                                                                .filter((e) => e.tipo !== "ganho" && e.tipo !== "perdido")
                                                                .map((e) => (_jsx("option", { value: e.id, children: e.nome }, e.id)))] }), erroReabertura && _jsx("p", { className: "side-panel__aviso", children: erroReabertura }), _jsx("button", { className: "side-panel__botao-primario", onClick: confirmarReabertura, disabled: reabrindo || !etapaReaberturaAlvo, children: reabrindo ? "Reabrindo\u2026" : "Reabrir oportunidade" })] })) : etapaEhFinal ? (_jsxs("p", { className: "side-panel__aviso", children: ["Etapa final (", etapaAtual?.nome, ") \u2014 n\u00E3o pode ser alterada."] })) : (_jsxs("div", { className: "side-panel__form", children: [_jsxs("select", { value: etapaAlvo, onChange: (e) => {
                                                            ...
                                                            setMotivoOutroAlvo("");
                                                        }, children: [_jsx("option", { value: "", children: "Selecione o motivo da perda\u2026" }), motivosPerda.map((m) => (_jsx("option", { value: m.id, children: m.nome }, m.id)))] })), precisaOutroTexto && (_jsx("input", { type: "text", placeholder: "Descreva o motivo\u2026", value: motivoOutroAlvo, onChange: (e) => setMotivoOutroAlvo(e.target.value) })), precisaVisita && (_jsxs("label", { className: "side-panel__campo", children: [_jsx("span", { children: "Data e horário da visita *" }), _jsx("input", { type: "datetime-local", value: visitaAlvoData, onChange: (e) => setVisitaAlvoData(e.target.value) })] })), erroMovimento && _jsx("p", { className: "side-panel__aviso", children: erroMovimento }), _jsx("button", { className: "side-panel__botao-primario", onClick: confirmarMovimento, disabled: movendoEtapa ||
                                                            !etapaAlvo ||
                                                            (precisaMotivo && !motivoAlvo) ||
                                                            (precisaOutroTexto && !motivoOutroAlvo.trim()) ||
                                                            (precisaVisita && !visitaAlvoData), children: movendoEtapa ? "Movendo…" : "Confirmar movimentação" })] }))] }), _jsxs("div", { className: "side-panel__secao", children: [_jsx("h3", { className: "side-panel__secao-titulo", children: "Transferir para outro usu\u00E1rio" }), _jsxs("div", { className: "side-panel__form", children: [_jsxs("select", { value: responsavelAlvo, onChange: (e) => {
